<?php	
	...
	$sql="SELECT Table_priv from tables_priv where Table_name='ticket' and user='$name';";
	...
	$sql="SELECT Select_priv, Update_priv from db where user='$name';";
	...
	$row=$result->fetch();
	$row1=$result1->fetch();
	if((!strstr($row['Table_priv'],'Update') && !strstr($row1['Update_priv'],'Y')) || !strstr($row1['Select_priv'],'Y'))
